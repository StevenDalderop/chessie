...
import { makeAuthorizedFetch } from "../functions"
...
class Game extends React.Component {
  ...
  async make_move_or_promotion(move) {
    ...
    check_promotion(move) {
      ...
      makeAuthorizedFetch(`${baseURL}/api/check_move_valid/${uci_promotion}`)
		.then(res => res.json())
		.then(data => {
		  if (data["valid"] === "true") {
  ...
  make_move(uci) {
    makeAuthorizedFetch(`${baseURL}/api/make_move/${uci}`)	
		.then(res => res.json())
		.then(data => {
			if (data["valid"] === "true") {
			  this.setState((state) => ({
  ...
  get_pc_move() {
	makeAuthorizedFetch(`${baseURL}/api/get_pc_move`)
  ...
  handleResignButtonPressed() {
	  ...
	  makeAuthorizedFetch(`${baseURL}/api/me/games/resign`)
  }

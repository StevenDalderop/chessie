...
import { makeAuthorizedFetch, fetchDataURL } from "../functions"
...
const fetchApiCreateGame = (vs : string, time : number, skill_level : number = null) => {	
	...
	return makeAuthorizedFetch(`/api/me/games`, json)
				.then(res => res.json())
}

const fetchApiLeaveGames = () => {
	return makeAuthorizedFetch(`${baseURL}/api/me/leave-games`)
}	
...
const fetchApiJoinGame = (game_id : number) => {
	...
	return makeAuthorizedFetch(`/api/games/${game_id}/join-game`, json)
				.then(res => res.json())
}
	
const GameSettings: React.FC<{username: string, handleError: any}> = (props) =>  {
	...
	const pageSwitch = (page: string) : React.ReactNode => {
		switch (page) {
			...
			case pages.online:
				return <OnlineGame 
							handleError={(error : any) => props.handleError(error)}
		...
		<div>			
			<Switch>
				...
				<Route exact path="/results">
					<Results username={props.username} handleError={(error : any) => props.handleError(error)}/> 
				</Route>
				<Route path="/">
					<NotFound />
				</Route>
